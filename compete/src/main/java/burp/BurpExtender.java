...
public class BurpExtender implements IBurpExtender, ITab, IContextMenuFactory {
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            Future<?> future = scheduler.schedule(() -> {
                try {
                    ...
                    logResponse(response, requestId, requestTime);
     ...
     */
    private void logResponse(IHttpRequestResponse response, int requestId, String requestTime) {
        SwingUtilities.invokeLater(() -> {
            String responseTime = TIME_FORMATTER.format(LocalDateTime.now());
            try {
                ...
                competeGUI.appendResponse(
                        String.format("[请求%d][请求:%s][响应:%s] 状态码:%s\n%s\n",
                                requestId, requestTime, responseTime, statusCode, body)
                );
            } catch (Exception e) {
                competeGUI.appendResponse(
                        String.format("[请求%d][请求:%s][响应:%s] 响应解析异常\n", 
                            requestId, requestTime, responseTime)
                );
            }
        });
    }
